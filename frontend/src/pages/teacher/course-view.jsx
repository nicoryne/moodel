...
export default function TeacherCourseView() {
  ...
  const pendingStudents = courseDetails.course.enrolledStudents.filter((student) => student.isVerified === false)
              ...
              <ul>
                {activeStudents.map((data, index) => (
                  <li key={index} className="flex place-items-center gap-1">
                    <img src={temp_image} className="h-auto w-6 rounded-full" alt={data.student.fname + "Profile"} />
